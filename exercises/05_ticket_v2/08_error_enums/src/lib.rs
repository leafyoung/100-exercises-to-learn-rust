...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        ...
        Err(err) => match err {
            ...
            TicketNewError::LongTitle => panic!("{}", "Title cannot be longer than 50 bytes"),
            TicketNewError::EmptyDescription => Ticket::new(
                title.clone(),
                "Description not provided".into(),
                status.clone(),
            )
            .unwrap(),
            TicketNewError::LongDescription => Ticket::new(
                title.clone(),
                "Description not provided".into(),
                status.clone(),
            )
